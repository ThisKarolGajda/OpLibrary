...
public record OpStringComponent(String string) {
    ...
    public @NotNull TextComponent getHover() {
        ...
        Map<String, TextComponent> tempMap = new HashMap<>();
        ...
        for (String s : tempString.split(" ")) {
            tempMap.keySet().stream().filter(s::contains).findAny().ifPresentOrElse(group -> {
                ...
                if (textBefore.length() != 0) {
                    temp.addExtra(FormatTool.formatMessage(textBefore));
                }
                ...
                if (textAfter.length() != 0) {
                    temp.addExtra(FormatTool.formatMessage(textAfter) + " ");
                }
            }, () -> temp.addExtra(FormatTool.formatMessage(s) + " "));
        }
    ...
    public @NotNull TextComponent getTextComponent(String[] tempString, Map<String, TextComponent> tempMap) {
        TextComponent temp = new TextComponent();
